...
.venv/bin/python scripts/record_reward_trials.py --mock --sessions 3 --trials 120 \
  --signal-gain 0.6 --stimuli-dir data/stimuli \
  --out data/reward_training/pref_trials.csv
```

`data/stimuli/` and `data/targets_real/` are local, gitignored datasets. Populate
them before recording; image fixtures and generated candidates are not stored in
the repository.
...
```bash
.venv/bin/python scripts/train_reward_model.py \
  --data data/reward_training/ --model lda --out models/preference_model.joblib
```
